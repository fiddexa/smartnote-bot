    ...
    try:
        ...
        if image_count == 1:
            ...
            await message.reply_text(
                ...
                f"📚 <b>Получено фотографий: {image_count}</b>\n\n"
        ...
        def recognize_images():
            ...
            prompt = f"""
Ты работаешь как OCR-модуль SmartNote AI.

Перед тобой {image_count} фотографий.

ОЧЕНЬ ВАЖНО:

Все изображения являются страницами ОДНОГО учебного
материала.

Ты должен обработать ВСЕ изображения В ОДНОМ ЗАПРОСЕ.

Не обрабатывай их как отдельные документы.

Порядок изображений соответствует порядку страниц:
...
1. Распознать текст со ВСЕХ фотографий.

2. Объединить весь текст в ОДИН материал.

3. Сохранить правильный порядок страниц.

4. Если предложение начинается на одной странице
   и продолжается на другой — объединить его.
...
6. Если есть таблицы — передай их содержание
   в понятном текстовом виде.

7. Если есть несколько колонок —
   правильно определить порядок чтения.

8. Если текст плохо читается —
   не выдумывай его.
...
13. Не сокращай материал.

14. Не повторяй страницы.

15. Не разделяй результат на отдельные ответы
    для каждой фотографии.

16. Верни ОДИН единый текст всего материала.

Верни только распознанный материал.
"""
            ...
            print(
                "🚀 ОТПРАВЛЯЮ ВСЕ ФОТО GEMINI ОДНИМ ЗАПРОСОМ",
                flush=True
            )
