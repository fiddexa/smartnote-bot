...
Перед тобой {image_count} изображений.

КРИТИЧЕСКИ ВАЖНО:

Все изображения являются страницами ОДНОГО
учебного материала.

Ты должен обработать ВСЕ {image_count} изображений
В ОДНОМ ЗАПРОСЕ.

НЕЛЬЗЯ:

- обрабатывать страницы как отдельные документы;
- делать отдельный ответ для каждой фотографии;
- повторять одинаковый текст;
- пропускать страницы;
- сокращать текст;
- создавать реферат;
- создавать конспект;
- создавать выжимку;
- придумывать отсутствующий текст.

ПОРЯДОК:

Изображение 1 = первая страница.

Изображение 2 = следующая страница.

Изображение 3 = следующая страница.

И так далее.

Объедини всё в ОДИН последовательный материал.

Если предложение начинается на одной странице
и продолжается на следующей — правильно объедини
его.

Если абзац продолжается на следующей странице —
не начинай новый смысловой блок без необходимости.
...
Если на фотографии есть таблица —
передай её содержание максимально точно
в текстовом виде.

Если есть несколько колонок —
определи правильный порядок чтения.

Если отдельный фрагмент невозможно прочитать,
НЕ ПРИДУМЫВАЙ его.
...
И ОБЪЕДИНИТЬ ЕГО В ОДИН МАТЕРИАЛ.
...
"""
            ...
            if not response:
                ...
                raise RuntimeError(
                    "Gemini не вернул response."
                )

            return response.text

        # =====================================================
        # ЗАПУСК GEMINI В ОТДЕЛЬНОМ ПОТОКЕ
        # =====================================================
        ...
        if not material:
            ...
            raise RuntimeError(
                "После очистки текст оказался пустым."
            )
